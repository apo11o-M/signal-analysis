...
struct RxConfigChirpDopplerBank {
    ...
    double doppler_search_hz = 0.0; // search +/- this range (0 = disable doppler bank)
    double doppler_oversample = 1.0; // bin spacing = (fs / L) / oversample
    std::size_t doppler_max_bins = 401;  // allow up to 401 bins total to prevent silly sizes
...
struct RxResultsChirpDopplerBank : public RxResults {
    bool detected = false;
    ...
    ~RxResultsChirpDopplerBank() override = default;
...
class ReceiverChirpDopplerBank : public Receiver {
private:
    struct MatchFilterResult {
        ...
        // doppler bank outputs
        double best_doppler_hz = 0.0;
...
public:
    ...
    ReceiverChirpDopplerBank(const RxConfigChirpDopplerBank& config) 
    ...
    std::unique_ptr<RxResults> process_frame(const Frame& frame) {
        std::unique_ptr<RxResultsChirpDopplerBank> res = std::make_unique<RxResultsChirpDopplerBank>();
...
private:
    inline cfloat phasor_neg_(double phase_rad) const {
        // e^{-j phase} = cos(phase) - j sin(phase)
        return cfloat(static_cast<float>(std::cos(phase_rad)),
    ...
    MatchFilterResult match_filter_doppler_(const Frame& frame, std::size_t max_tau_ext, double doppler_hz) {
        const std::size_t L = config_.duration_sample;
        const double fs = config_.common.sample_rate_hz;
        ...
        double best_mag = -1.0;
        std::size_t best_tau_ext = 0;
        // constant used in phase = 2pi * f * (m / fs)
        const double w = 2.0 * M_PI * (doppler_hz / fs);

        for (std::size_t tau = 0; tau <= max_tau_ext; tau++) {
            cfloat acc_corr(0.0f, 0.0f);

            for (std::size_t n = 0; n < L; n++) {
                const std::size_t m = tau + n;
                const cfloat x = sample_ext_(frame, m);
                const cfloat s = replica_chirp_[n];

                // mix down by doppler_hz using the extended index m
                const double phase = w * static_cast<double>(m);
                const cfloat mix = phasor_neg_(phase);
                acc_corr += (x * mix) * std::conj(s);   // apply the phase to our signal x
            }
        ...
        res.best_mag = best_mag;
        res.mean_mag = mean_mag;
        res.best_doppler_hz = doppler_hz;
        res.best_ratio = best_mag / (mean_mag + EPSILON);
        return res;
    }

    MatchFilterResult doppler_bank_acquire_(const Frame& frame, std::size_t max_tau_ext) {
        const std::size_t L = config_.duration_sample;
        const double fs = config_.common.sample_rate_hz;

        // If doppler bin is disabled, just run the original 1D match filter at doppler=0
        if (config_.doppler_search_hz <= 0.0) {
...
private:
    RxConfigChirpDopplerBank config_;
